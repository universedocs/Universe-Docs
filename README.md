...
Stores document items in natural and hierarchial order so that user can understand its nature! Document item is nothing but a unique text or photo or video or etc., This document item document is opened when a user selects "Show Graph" from a document item for example "Trichy"!
